...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 18); // VirtualCoin: starting difficulty is 1 / 2^18
...
int64 static GetBlockValue(int nBits, int nHeight, int64 nFees)
{
    ...
    int64 nSubsidy = 50; 
    if(nHeight >= 100) {
        if((nHeight >= 1000 && dDiff > 75) || nHeight >= 2000) { // GPU/ASIC difficulty calc
            // 2222222/(((x+2600)/9)^2)
    ...
    // yearly decline of production by 20% per year, projected 21.3M coins max by year 20XX.
...
static const int64 nTargetSpacing = 30 * 60; 
static const int64 nInterval = nTargetTimespan / nTargetSpacing; // 48
...
unsigned int static VirtualGravityWell(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
{ DiffMode = 3; }
        ...
        else if (DiffMode == 2) { return VirtualGravityWell(pindexLast, pblock); }
        else if (DiffMode == 3) { return VirtualGravityWell(pindexLast, pblock); }
        return VirtualGravityWell(pindexLast, pblock);
}
...
bool SetBestChain(CValidationState &state, CBlockIndex* pindexNew)
{
    ...
    if (!fIsInitialDownload)
    {
        ...
        if (nUpgraded > 100/2)
            ...
            strMiscWarning = _("Warning: This version is no longer supported, upgrade to new version");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        if (fTestNet)
        {
            block.nTime = 1396926000;
            block.nBits = 0x1e0ffff0;
            block.nNonce   = 1643023;
        }
...
string GetWarnings(string strFor)
{
    ...
    {
        nPriority = 2000;
        strStatusBar = strRPC = _("Warning: Displayed transactions may not be correct! You may need to upgrade to new vCoin version from vCoin.ca");
    }
