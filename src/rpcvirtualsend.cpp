...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (strCommand == "count") return (int)virtualSendMasterNodes.size();
    ...
    if (strCommand == "current")
    {
        ...
        int winner = virtualSendPool.GetCurrentMasterNode(mod);
        if(winner >= 0) {
            return virtualSendMasterNodes[winner].addr.ToString().c_str();
        }
    ...
    {
        Object obj;
        BOOST_FOREACH(CMasterNodeVote& mv, virtualSendMasterNodeVotes) {
