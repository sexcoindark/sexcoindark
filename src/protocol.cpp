...
// Virtualcoin message start (switch from Litecoin's)
static unsigned char pchMessageStartLitecoin[4] = { 0xfb, 0xc0, 0xb6, 0xdb };
static unsigned char pchMessageStartVirtualcoin[4] = { 0xfb, 0xc0, 0xb6, 0xdb };
...
void GetMessageStart(unsigned char pchMessageStart[], bool fPersistent)
{
    if (fTestNet)
        ...
        memcpy(pchMessageStart, (fPersistent || GetAdjustedTime() > nMessageStartSwitchTime)? pchMessageStartVirtualcoin : pchMessageStartLitecoin, sizeof(pchMessageStartVirtualcoin));
}
